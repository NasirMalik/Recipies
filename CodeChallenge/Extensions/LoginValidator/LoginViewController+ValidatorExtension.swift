...
extension LoginViewController
{
    ...
    func setValidators() {
        ...
        let emailRule = ValidationRulePattern(pattern: emailPattern, error: ValidationError(message: "Invalid Email"))
        ...
        let minLengthRule = ValidationRuleLength(min: 6, error: ValidationError(message: "Min 6 chracters"))
    ...
    func updateValidationStatusForButton() -> Bool {
        if emailTextField.validate(rules: emailvalidationRuleSet!).isValid && passwordTextField.validate(rules: passwordvalidationRuleSet!).isValid{
            return true
        }
        else{
            return false
